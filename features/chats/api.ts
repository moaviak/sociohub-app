...
import { addChat, setChats, setMessages } from "./slice";
...
export const chatsApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    sendMessage: builder.mutation<
      Message,
      { chatId: string; content?: string; formData?: FormData }
    >({
      query: ({ chatId, content, formData }) => {
        console.log("Sending message to:", `/messages/${chatId}`);

        // If FormData is provided, use it (for attachments)
        if (formData) {
          console.log("Using FormData for message with attachments");
          return {
            url: `/messages/${chatId}`,
            method: "POST",
            body: formData,
